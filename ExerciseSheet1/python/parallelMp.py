...
#     v1 = np.random.randint(size, size=int(size))
...
def MiniumNumber(v, pos_ini, pos_fin, RES_MIN):
    for pos_ini in range(pos_ini, pos_fin):
        if v[pos_ini] < RES_MIN.value:
            lock.acquire()
            RES_MIN.value = v[pos_ini]
            lock.release()
...
# ------------------- EXPERIMENT b) ------------------------------      
    ...
    RES_MIN = multiprocessing.Value('d', 0.0)
    ...
    for nt in range(nt, NUM_TH+1): 
        ...
        outfile.write(str(tam) + '\t' + str(RES_MIN.value) + '\t' + str(tim) + '\t' + str(nt) + '\n') 
    outfile.close()
#     #      
# # ------------------- EXPERIMENT c) ------------------------------    
#      
