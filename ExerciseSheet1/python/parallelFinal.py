...
# ------------------- EXPERIMENT b) ------------------------------      
    ...
    for nt in range(nt, nt+1): 
        ...
        for i in range(nt):
            ...
            t = threading.Thread(target=parMin.MiniumNumber_nogil, args=(pos_ini, pos_fin, min_aux[0]))
            pos_ini = pos_fin # update initial position for next thread
            RES_MIN = parMin.getGlobal()
            t.start()
            t.join()
...
#                 pos_fin = pos_ini+1
#                 t = threading.Thread(target=parMin.MiniumNumber_nogil, args=(v2, pos_ini, pos_fin))
#                 pos_ini = pos_fin
#                 t.start()
#                 t.join() 
